...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < self.presenter.cellPresenters.count) {
        ...
        cell.presenter = cellPresenter;  // P-V 绑定
        ...
        cell.likeHandler = ^{
            ...
            // 实际的点赞逻辑调用的还是 P 层实现
            // 因为现在 M 层的处理逻辑都交给了 P 层，所以，所有有关 M 的修改都必须要过 P 层
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.eventHandler) {
        self.eventHandler(NSStringFromSelector(@selector(tableView:didDeselectRowAtIndexPath:)), self.presenter.cellPresenters[indexPath.row].blog);
    }
}
